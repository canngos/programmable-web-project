"""
Booking routes for the Flight Management System API.
...
def create_booking(current_user):
    """Create a booking and one or more tickets for the authenticated user."""
    try:
...
def list_bookings(current_user):
    """List bookings for the current user. Admin can list all with ?all=true."""
    try:
...
def get_booking(current_user, booking_id):
    """Get a booking by ID for owner or admin."""
...
def get_seat_availability(current_user):
    """Check if a seat is available for booking on a given flight."""
    try:
